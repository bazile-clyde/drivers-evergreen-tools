...
(function() {
   ...
   function isUbuntu1804() {
      ...
      const grep_result =
          runProgram('grep', 'UBUNTU_CODENAME=bionic', '/etc/os-release');
      if (grep_result === 0) {
         return true;
      }
   ...
   // Since the container is Ubuntu 18.04, it does to make sense to run binaries
   // from other distros on
   // it.
   if (!isUbuntu1804()) {
      console.log("Script must be run on Ubuntu 18.04");
      return;
   }
   ...
   const mongo_binaries = process.env.MONGODB_BINARIES;
   const project_dir = process.env.PROJECT_DIRECTORY;
